...
public class DayStudyQuestionAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private RealmList<DayStudyQuestionSunji> sunjiList;
    private DayStudyQuestionSunji sunjiData;
    private DayStudyQuestion questionData;
    ...
    public interface OnItemSelectListener { // create an interface
        void onItemSelect(DayStudyQuestion questionData, int position); // create callback function
    }

    public DayStudyQuestionAdapter(Context mContext, RealmList<DayStudyQuestionSunji> sunjiList,
                                   OnItemSelectListener listener) {
        this.mContext = mContext;
        this.sunjiList = sunjiList;
        questionData = sunjiList.first().getQuestionList().first();
        this.listener = listener;
    }

    public DayStudyQuestionAdapter(Context mContext, RealmList<DayStudyQuestionSunji> sunjiList, DayStudyQuestion questionData,
                                   OnItemSelectListener listener) {
        this.mContext = mContext;
        this.sunjiList = sunjiList;
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        ...
        // 문제 제출 후
                ...
                if ((sunjiExplainPointEnd.size() == sunjiExplainPointStart.size()) && (sunjiExplainPointEnd.size() != 0)) {
                    ...
                    for (int i = 0; i < sunjiExplainPointEnd.size(); i++) {
                        if (sunjiExplainPointEnd.get(i) > sunjiExplainPointStart.get(i)) {
                            sunjiExplainSpannable.setSpan(new BackgroundColorSpan(ContextCompat.getColor(mContext, R.color.col_80FFE0B2)), sunjiExplainPointStart.get(i), sunjiExplainPointEnd.get(i),
                                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                        }
                    }
            ...
            String correct = sunjiData.getIpaType();//O 정답, X:오답

            if (null != questionData.getSolved() && questionData.getSolved().equalsIgnoreCase("Y") && !TextUtils.isEmpty(questionData.getSelectSunji())) {
                setSunjiList(genericHolder, position, correct);
    ...
    private void setSunjiList(GenericViewHolder genericHolder, int position, String correct) {//correct : O 정답, X:오답
        int selectSunji = Integer.parseInt(questionData.getSelectSunji());
    ...
    public DayStudyQuestion getQuestionData() {
        return questionData;
    }

    public String getQuestionStudyType() {
        return questionData.getStudyType();
    }
}